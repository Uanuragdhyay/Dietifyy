...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  void _editDOB() async {
    ...
    if (pickedDate != null) {
      await _firestore
          ...
          .update({'dob': Timestamp.fromDate(pickedDate)});
      setState(() {
  ...
  void _editPreferences() {
    ...
    List<String>.from(userData?['dietaryChoices'] ?? []);
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setStateModal) => Padding(
          ...
          child: Column(
            ...
            children: [
              const Text("Select Dietary Preferences",
              ...
              ElevatedButton(
                onPressed: () async {
                  await _firestore
                      .collection('users')
                      .doc(_auth.currentUser!.uid)
                      .update({'dietaryChoices': selectedPreferences});
                  setState(() {
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildInfoRow("Gender", userData!['gender'] ?? 'Not Set',
                              () => _showEditDialog("Gender", userData!['gender'] ?? '')),
                      _buildInfoRow(
                        "DOB",
                        userData!['dob'] != null
                      ...
                      _buildInfoRow(
                        "Preferences",
                        (userData!['dietaryChoices'] as List<dynamic>?)?.join(", ") ??
                            "Not Set",
                        _editPreferences,
                      ),
                      _buildInfoRow("Allergies",
                          userData!['allergies'] ?? "Not Set",
                              () => _showEditDialog("Allergies", userData!['allergies'] ?? '')),
                    ],
                  ),
                ),
              ),
